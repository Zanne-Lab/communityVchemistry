...
```{r}
  ...
  ggplot(tmp, aes(x = codeStem, y = diff)) +
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 1))
```  
Go back and investigate the JASC/Arabinose and ACPA/Xylose outliers. For now, just average the analytical reps
...
(1) Original traits: "Ca","Fe","K","Mn","N","P","Zn","waterperc" predict percent mass remaining (PMR) at each time point
```{r}
# rhs.orig model
rhs.orig <- paste(orig.x.vars, collapse = " + ")
...
(2) Original traits plus C fractions...
```{r}
# rhs.orig model
rhs.plusC <- paste(c(orig.x.vars, cfract.x.vars), collapse = " + ")
