...
#Fig S1
makefig__compare_t50(decayfits) # output/figures/supplementary/compare_t50.pdf
...
#Fig S6
makefigs__endoComp_woodTraits(comm.otu, seqSamples, traits.code, use.cache = T) # output/figures/supplementary/dbRDA_code.pdf
...
makefigs__endoComp_woodTraits.stem(comm.otu, traits.code, traits.stem, use.cache = T) # output/figures/supplementary/dbRDA_stem.pdf
tab.list <- maketabs__endoComp_woodTraits.stem(comm.otu, traits.code, traits.stem, use.cache = T)
...
rhs <- "waterperc + N + Arabinose + Galactose + Mannose + perc.Total.lignin + Rhamnose" # got rid of size because only looking at large stems
...
- alpha: Codes that decay quickly (at first) have more water and Rhamnose
- beta: Codes with s-shaped trajectories have less water, more Rhamnose, larger fraction of total lignin
- w.r2: Codes with better model fits have more water and less lignin
- w.t50: Codes with longer half lives have more water, more lignin, more Rhamnose
...
- waterperc is important for explaining pmr at each time point. Stems with low waterperc tend to have more mass remaining.
- Low Rhamnose, Mannose and Arabinose correspond with more mass remaining early-on
- High Rhamnose and lignin correspond with more mass remaining at later timepoints
...
Does adding C fraction data explain more variation in endophyte composition? How much?
```{r}

xVars <- traits.tmp
xVars %>%
  select(-c(codeStem, code, size, species, barkthick, density)) %>%
  filter(!is.na(waterperc)) -> envVars
