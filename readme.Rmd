...
ord<-match(row.names(codeOTUabund), decayfits.trim$code)
decayfits.trim.o<-decayfits.trim[ord,]
sum(row.names(codeOTUabund) != decayfits.trim.o$code) #this needs to be 0

#fit models
respVars <- list("k","t70","ne.r2")
cvfit.notrim<-lapply(respVars, function(x) {fitNcrossval_WAPLS(curr.comm = codeOTUabund, curr.respVar = decayfits.trim.o[[x]])}) #analysisDF_fxns.R
cvfit.trim<-lapply(respVars, function(x) {fitNcrossval_WAPLS(curr.comm = codeOTUabund.trim, curr.respVar = decayfits.trim.o[[x]])}) #analysisDF_fxns.R
...
rm(decayfits.trim, respVars, 
   ...
   prettyTab.notrim, prettyTab.trim, decayfits.trim.o, ord)
...
write.csv(summ.df, file="output/diversitysummary_stem.csv")
...
write.csv(summ.df, file="output/diversitysummary_code_traitresid.csv")
...
write.csv(summ.df, file="output/diversitysummary_stem_traitresid.csv")
...
datasets.notrim<-CreateCommTraitpair(comm.otu = codeOTUabund, traits=traits.noNAs, sampleName = "code")
datasets.trim<-CreateCommTraitpair(comm.otu = codeOTUabund.trim, traits=traits.noNAs, sampleName = "code")

# choose a model by permutation tests in a constrained ordination

cap.env <- capscale(datasets.notrim[['comm']] ~ ., data = datasets.notrim[['traits']], distance='bray') # full model
mod0.env <- capscale(datasets.notrim[['comm']] ~ 1, data = datasets.notrim[['traits']], distance='bray') # set up the null cases with no predictors
step.env <- ordistep(mod0.env, scope=formula(cap.env)) # model selection
...
#fit models
dbrda.notrim.code<-capscale(datasets.notrim[['comm']] ~ ., data=datasets.notrim[['traits']], distance = "bray")
dbrda.trim.code<-capscale(datasets.trim[['comm']] ~ ., data=datasets.trim[['traits']], distance = "bray")
adonis.notrim.code<-adonis(datasets.notrim[['comm']] ~ ., data=datasets.notrim[['traits']], distance = "bray")
adonis.trim.code<-adonis(datasets.trim[['comm']] ~ ., data=datasets.trim[['traits']], distance = "bray")

#signif terms
dbrdaTerms.notrim.code<-anova(dbrda.notrim.code, by = "terms")
dbrdaTerms.trim.code<-anova(dbrda.trim.code, by="terms")
#adonis.notrim.code
#adonis.trim.code
...
#save summary tables
str(dbrdaTerms.notrim.code)

data.frame(term=row.names(dbrdaTerms.notrim.code), 
           df=dbrdaTerms.notrim.code$Df,
           Fval=round(dbrdaTerms.notrim.code$F, digits=2),
           pval=dbrdaTerms.notrim.code$`Pr(>F)`)
adonis.notrim.code
...
# make pair of matching datasets with the community mat and the trait mat
datasets.notrim<-CreateCommTraitpair(comm.otu = comm.otu, traits = traits.stem.plus, sampleName = "codeStem")
datasets.trim<-CreateCommTraitpair(comm.otu = comm.otu.trimmed, traits = traits.stem.plus, sampleName = "codeStem")

#fit models
dbrda.notrim.stem<-capscale(datasets.notrim[['comm']] ~ ., data=datasets.notrim[['traits']], distance = "bray")
dbrda.trim.stem<-capscale(datasets.trim[['comm']] ~ ., data=datasets.trim[['traits']], distance = "bray")
adonis.notrim.stem<-adonis(datasets.notrim[['comm']] ~ ., data=datasets.notrim[['traits']], distance = "bray")
adonis.trim.stem<-adonis(datasets.trim[['comm']] ~ ., data=datasets.trim[['traits']], distance = "bray")

#signif terms
dbrdaTerms.notrim.stem<-anova(dbrda.notrim.stem, by = "terms")
dbrdaTerms.trim.stem<-anova(dbrda.trim.stem, by="terms")
adonis.notrim.stem
adonis.trim.stem
