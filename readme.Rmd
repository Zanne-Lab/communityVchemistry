...
fit.tr.t13.nt <- WAPLS(dataset.t13.notrim[['comm']], dataset.t13.notrim[['traitresid']]$trait.resid, npls=20)
fit.tr.t25.nt <- WAPLS(dataset.t25.notrim[['comm']], dataset.t25.notrim[['traitresid']]$trait.resid)
fit.tr.t37.nt <- WAPLS(dataset.t37.notrim[['comm']], dataset.t37.notrim[['traitresid']]$trait.resid, npls=20)
...
rand.t.test(fit.tr.t13.cv.nt) # comp2 has a signif pval, but this dimension's RMSE and R2 values have dropped, which suggests that looking at this component is not warrented... doesn't seem to be contributing to a better global fit overall
#rand.t.test(fit.tr.t25.cv) 
#rand.t.test(fit.tr.t25.cv.nt)
#rand.t.test(fit.tr.t37.cv) 
rand.t.test(fit.tr.t37.cv.nt) # comp2 has a signif pval, but this dimension's RMSE and R2 values have dropped, which suggests that looking at this component is not warrented... doesn't seem to be contributing to a better global fit overall
