...
probably want to use weibull instead?
```{r, echo = F}

#make plot of decay params by wood species
indx <- unique(stemSamples[,c("code","species","size","Family","Binomial","site")])
decayfits %>%
  left_join(indx) -> decayfits.p

# t70
p.t70 <- ggplot(decayfits.p, aes(x=reorder(Binomial, -t70), y=t70, color=size)) + 
  geom_point(size = 3, alpha = .5) +
  #geom_errorbar(aes(ymin=t70.lower, ymax=t70.upper)) +
  coord_flip() +
  xlab("Wood species") + ylab("Years to 70% mass remaining") + theme_bw() +
  scale_color_manual(values=c('large'="red", 'small'="black")) + 
  guides(color = F) +
  theme(plot.margin = unit(c(0,1,0,0), "lines"))

# k
p.k <- ggplot(decayfits.p, aes(x=reorder(Binomial, -t70), y=k, color=size)) + 
  geom_point(size = 3, alpha = .5) +
...
# ne.r2
p.r2 <- ggplot(decayfits.p, aes(x=reorder(Binomial, -t70), y=ne.r2, color=size)) + 
  geom_point(size = 3, alpha = .5) +
  coord_flip() +
...
p.pmrBystem <- ggplot(pmr.byStem.df, aes(x=timeNum/12, y=mean.pmr*100, group=codeStem, linetype=size)) + 
...
p.denVar<-ggplot(db.df, aes(x=reorder(code, density_code), y=density_stem)) + 
  geom_point(size = 3, alpha = .5) + theme_bw() + coord_flip() +
...
```{r, echo = F}
#Plots based on significant relationships prior to adding time59
