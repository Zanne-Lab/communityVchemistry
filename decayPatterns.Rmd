...
source("code/summaryTable_fxns.R") #for list_to_df()
...
```{r}
ggplot(decayfits,aes(x=t60,y=w.t60,col=size))+
  geom_point()+
  labs(x="Time to 40% mass loss (negative exponential)",
       y="Time to 40% mass loss (Weibull)")+
...
ggplot(plot.df, aes(x = variable, y = reorder(Binomial, value), 
  ...
  guides(fill = F)
#ggsave(filename = "output/compare_t60.pdf", width = 5, height = 6)
