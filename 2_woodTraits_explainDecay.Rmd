...
- Model fits: The best fitting models used traits to explain w.t50 and beta (scale), both with R2 > 0.70.

- Water content was the most influential trait on time to 50% mass loss; more water was associated with short wait times (fast decay). The role of water mainly appeared to influence the scale parameter; more water led to lower scale parameter values (faster decay)
```{r}
...
Plot the effect of waterperc on the scale parameter
```{r}
# result.traitsDecay$preddat$waterperc_s %>%
#   filter(respvar == c("beta")) %>%
#   dplyr::rename('beta'='respval') %>%
#   select(beta, size, waterperc, .fitted) -> plot.df
# plot.df$code <- result.traitsDecay$data$code
# plot.df %>%
#   left_join(pmr) -> plot.df
# quant <- quantile(plot.df$waterperc, probs = c(.25,.5,.75))
# quant
# plot.df %>%
#   mutate(level = ifelse(waterperc <= quant[[1]], "0-25th", 
#                                   ifelse(waterperc > quant[[1]] & waterperc <= quant[[2]], "25-50th",
#                                          ifelse(waterperc > quant[[2]] & waterperc <= quant[[3]], "50-75th",
#                                                 ifelse(waterperc > quant[[3]], "75-100th", NA))))) -> plot.df
# # fit weibull model for each size and level
# weibull.fxn <- function(x, beta, alpha){
#     y = exp(-(x/beta)^alpha)
#     return(y)
# }
# recalc.fits <- function(data){
#   
#   max.time <- 6
...
#   # small
#   data %>%
#     filter(size == "small") -> data.s
#   param.list <- df.list <- list()
#   for(i in 1:length(LEVELs)){
#     data.s %>%
#       filter(level == LEVELs[i]) -> tmp
#     mod <- fit_litter(time = tmp$time/12, mass.remaining = tmp$pmr, model = "weibull")
#     param.list[[i]] <- data.frame(beta = mod$optimFit$par[1], alpha = mod$optimFit$par[2])
#     time.vec <- seq(0, max.time, 1e-01)
#     y.pred <- predict(mod, time.vec)
#     df.list[[i]] <- data.frame(time.vec, y.pred)
#   }
#   names(param.list) <- names(df.list) <- LEVELs
#   df <- list_to_df(df.list)
#   df %>%
#     dplyr::rename('level'='source') %>%
#     mutate(size = "small") -> df.s
#   
#   # large
#   data %>%
#     filter(size == "large") -> data.l
#   param.list <- df.list <- list()
#   for(i in 1:length(LEVELs)){
#     data.l %>%
#       filter(level == LEVELs[i]) -> tmp
...
# }
# 
# pred.df <- recalc.fits(data = plot.df)
# 
# ggplot(data = plot.df, aes(x = time/12, y = pmr*100, color = size)) +
#   geom_point() +
#   geom_line(data = pred.df, aes(x = time.vec, y = y.pred*100, color = size), 
#             inherit.aes = F) +
#   facet_grid(size~level) +
#   theme_bw() +
#   xlab("Time (yrs)") + ylab("Mass remaining (%)")
```

- Ca concentration had a large influence on t50 and the scale parameter; more Ca led to slower decay. 
```{r}
...
- The largest influence on alpha was diameter size; large sized stems tended to decay with a more S-shaped rate, but see EUSC and EUTE. 
```{r}
decayfits %>%
...
p.size <- ggplot(sel.decay, aes(x = size, y = alpha, color = species)) +
  ...
  ylab("Shape parameter") + xlab("Size")
p.size
...
pdf("output/figures/variation_densityNbarkthick.pdf", width=5, height=5)
  ...
  grid.draw(cbind(ggplotGrob(p.list$p.denVar), 
                  ggplotGrob(p.list$p.barVar), size = "last"))
dev.off()
...
# ggsave(filename = "output/figures/bivariate_traits-code.pdf",
#        plot = bivariate_explainDecay.list$traits.code, width = 10, height = 10)
# ggsave(filename = "output/figures/bivariate_traits-stem.pdf",
#        plot = bivariate_explainDecay.list$traits.stem, width = 12, height = 12)
# ggsave(filename = "output/figures/bivariate_cfract-code.pdf",
#        plot = bivariate_explainDecay.list$cfract.code, width = 8, height = 8)
# ggsave(filename = "output/figures/bivariate_cfract-stem.pdf",
#        plot = bivariate_explainDecay.list$cfract.code, width = 8, height = 8)
```
...
x.c <- models_explainDecay.list$traits.code$p +
  theme(axis.title.x=element_blank(),
        ...
        axis.title.y=element_blank()) + guides(fill = F)
x.s <- models_explainDecay.list$cfract.stem$p +
  theme(axis.title.x=element_blank(),
        axis.text.y=element_blank(),
        axis.ticks.y=element_blank(),
        axis.title.y=element_blank()) + guides(fill = F)
...
pdf("output/figures/traits-cfract_explainDecay.pdf", width = 8, height = 8)
grid.arrange(p.y1, p.tc + guides(fill = F), p.ts + guides(fill = F), 
             p.y2, p.cc + guides(fill = F), p.cs + guides(fill = F), 
...
pdf("output/figures/traits-cfract_explainDecay_FILLGUIDE.pdf", width = 6, height = 6)
grid.arrange(tmp)
dev.off()
...
## water
pdf(file = "output/figures/traits-code_explainDecay_waterCa.pdf", width = 6, height = 6)
...
## size
pdf(file = "output/figures/traits-code_explainDecay_size.pdf", width = 4.5, height = 4.5)
...
pdf(file = "output/figures/traits-stem_explainDecay_water.pdf", width = 6, height = 4)
...
## lignin
pdf(file = "output/figures/cfract-code_explainDecay_lignin.pdf", width = 6, height = 6)
plots_explainDecay.list$cfract.code$plots.l[[1]]
dev.off()

## rhgl
pdf(file = "output/figures/cfract-code_explainDecay_rhgl.pdf", width = 6, height = 6)
...
## early
pdf(file = "output/figures/cfract-stem_explainDecay_early.pdf", width = 6, height = 4)
plots_explainDecay.list$cfract.stem$plots.early[[1]]
dev.off()

## mid
pdf(file = "output/figures/cfract-stem_explainDecay_mid.pdf", width = 6, height = 6)
...
## lignin
pdf(file = "output/figures/cfract-stem_explainDecay_lignin.pdf", width = 6, height = 4)
