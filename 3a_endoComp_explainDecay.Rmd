...
Test saprotrophs
```{r}
cvfit.results.code.sapro <- doAnalysis_endoComp_explainDecay(comm.otu.sapro, seqSamples, decayfits, code.respVars,
                                                       use.cache = F, save.cache = F)
cvfit.results.code.sapro$prettyTabs # will merge this with the other summary table to export as a csv in the next chunk

models_explainDecay.list[['comm-decay.code.sapro']] <- cvfit.results.code.sapro$prettyTabs
```
...
models_explainDecay.list[['comm-decay.stem.sapro']] <- cvfit.results.stem$prettyTabs
...
Test saprotrophs
```{r}
cvfit.results.code.sapro <- doAnalysis_endoComp_explainDecayResids(comm.otu.sapro, seqSamples, decayfits, 
                                                             code.respVars, 
                                                             traitResiduals.code = resids_explainDecay.list$traits.code,
                                                             use.cache = F, save.cache = F)
cvfit.results.code.sapro$prettyTabs
cvfit.results.code.sapro

models_explainDecay.list[['comm-decayResidtraits.code.sapro']] <- cvfit.results.code.sapro$prettyTabs
...
Test saprotrophs
```{r}
cvfit.results.stem.sapro <- doAnalysis_endoComp_explainPMRResids(comm.otu.sapro, 
                                                           pmr_byStem, 
                                                           stem.respVars, 
                                                           traitResiduals.stem =
                                                             resids_explainDecay.list$traits.stem)
cvfit.results.stem.sapro$prettyTabs

models_explainDecay.list[['comm-decayResidtraits.stem.sapro']] <- cvfit.results.stem.sapro$prettyTabs
```
...
models_explainDecay.list[['comm-decayResidcfract.code.sapro']] <- cvfit.results.code.sapro$prettyTabs
...
Test saprotrophs
```{r}
cvfit.results.stem.sapro <- doAnalysis_endoComp_explainPMRResids(comm.otu.sapro, pmr_byStem, 
                                                           stem.respVars, 
                                                           traitResiduals.stem =
                                                             resids_explainDecay.list$cfract.stem)
cvfit.results.stem.sapro$prettyTabs

models_explainDecay.list[['comm-decayResidcfract.stem.sapro']] <- cvfit.results.stem.sapro$prettyTabs
```

Full saprotroph community marginally significant for explaining variation at the last time point (p = 0.06, time59)
...
filenames<- paste0("output/tables/wapls_", names(models_explainDecay.list),".csv")
for(i in 1:length(models_explainDecay.list)){
  write.csv(models_explainDecay.list[[i]], file = filenames[[i]])
}

# save the plots that dig into models summary results
pdf(file = "output/figures/wapls_pmr37.pdf")
grid.arrange(plots_explainDecay.list$`comm-decay.stem`$p1,
             plots_explainDecay.list$`comm-decay.stem`$p2, ncol = 2)
dev.off()
...
pdf(file = "output/figures/water-comm-pmr37.pdf")
