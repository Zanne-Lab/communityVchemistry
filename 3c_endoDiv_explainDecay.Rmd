...
Contents:
1. Test role of endo diversity in predicting decay, *slice 1*. (Exclude C fraction data), include small stems
- CODE
- STEM
2. Test role of endo diversity in predicting decay, *slice 2*. Include C fraction data, exclude small stems
- CODE
- STEM
3. Test role of endo diversity in predicting decay *after* accounting for traits, *slice 1.* (Exclude C fraction data), include small stems
- CODE
- STEM
4. Test role of endo diversity in predicting decay *after* accounting for traits, *slice 2.* Include C fraction data, exclude small stems
- CODE
- STEM
...
Beyond wood trait data, saprotroph richness improves models for beta and t50. 
...
Higher saprotroph richness leads to smaller beta and t50 values (i.e. faster mass loss) than would be expected based on wood traits alone, especially in large stems.
...
# Diversity plus C fractions as a predictor

Note: Only for large stems

## *Hyp (species+size-level)* After accounting for variation in decay due to C fractions, average initial microbial diversity (richness, Shannon diversity, ... add phylogenetic diversity) will predict variation in decay model fit (r2), rate (k), and lagginess (alpha).
```{r}
names(resids_explainDecay.list)
cResiduals.code <- resids_explainDecay.list$cfract.code

result.list <- doAnalysis_endoDiv_explainDecayResids_cfract(taxAndFunguild, comm.otu, cResiduals.code, code.respVars)

result.list$pretty.df
write.csv(result.list$pretty.df, file="output/tables/diversitysummary_code_traitresid_cfract.csv")

#identify significant relationships
result.list$pretty.df %>%
  filter(term %in% c('mean')) %>%
  gather(key = "respvar", value = "coef", -c(source, term)) %>%
  mutate(signif = ifelse(grepl("*", coef, fixed = T), T, F)) %>%
  filter(signif == T)
...
## *Hyp (stem-level)* After accounting for variation in decay due to C fractions, initial microbial diversity (richness, Shannon diversity, ... add phylogenetic diversity) will predict variation in percent mass loss, esp. at early time points.
```{r}
names(resids_explainDecay.list)
cResiduals.stem <- resids_explainDecay.list$cfract.stem

result.list.stem <- doAnalysis_endoDiv_explainPMRResids_cfract(taxAndFunguild, comm.otu, cResiduals.stem, stem.respVars)
write.csv(result.list.stem$pretty.df, file="output/tables/diversitysummary_stem_traitresid_cfract.csv")

#identify significant relationships
result.list.stem$pretty.df %>%
  filter(term %in% c('sub_rich')) %>%
  gather(key = "respvar", value = "coef", -c(source, term)) %>%
  mutate(signif = ifelse(grepl("*", coef, fixed = T), T, F)) %>%
  filter(signif == T)
